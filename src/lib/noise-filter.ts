// Strip noise (cover/back slides, footers, watermarks, HTML artifacts) from raw markdown.
// Ported & enhanced from standardize-knowledge / noise_filter.py
...
export const DEFAULT_NOISE_PATTERNS: string[] = [
  // Footer / watermark / confidentiality
  String.raw`^\*?Amartha(\s+Confidential)?\*?$`,
  String.raw`^\*?CONFIDENTIAL\*?$`,
  String.raw`^\*?For\s+Greater\s+Purpose\*?$`,
...
export function cleanHtmlArtifacts(text: string): string {
  return text
    ...
    .replace(/&quot;/g, '"')
    .replace(/&#039;/g, "'")
    .replace(/<\/?u>/gi, '') // strip <u> and </u> tags
    .replace(/<\/?b>/gi, '**')
    .replace(/<\/?i>/gi, '*');
}
...
export function filterNoise(
  ...
  const rawCleaned = cleanHtmlArtifacts(markdown);
  const lineRe = new RegExp(config.patterns.map((p) => `(?:${p})`).join("|"), "i");

  let slides = splitSlides(rawCleaned);
  ...
  for (const slide of slides) {
    ...
    for (const line of slide.split("\n")) {
      ...
      if (trimmed && lineRe.test(trimmed)) {
        linesStripped += 1;
        continue;
      }
      kept.push(line);
